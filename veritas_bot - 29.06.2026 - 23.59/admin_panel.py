...
def show_ban_warning_list_panel(bot, chat_id, photo_message_id, page=1):
    ...
    if not users:
        ...
        for u in page_users:
            status = "🚫 Yasaklı" if u['is_banned'] else f"⚠️ Uyarı: {u['warnings']}/5"
...
def show_user_ban_management_panel(bot, chat_id, photo_message_id, uid):
    ...
    metin = (
        ...
        f"⚠️ *Uyarı Sayısı:* `{user['warnings']} / 5`\n"
...
def system_control_klavyesi():
    ...
    markup.add(
        ...
        types.InlineKeyboardButton("💳 Ödeme Yöntemleri Yönetimi", callback_data="admin_sys_payment_methods"),
...
def payment_methods_control_klavyesi():
    ...
    crypto_btn_text = "⚡ Kripto: AKTİF (Bakıma Al)" if crypto_status == "on" else "⚡ Kripto: BAKIMDA (Aktif Et)"
    shopier_btn_text = "🛍️ Shopier: AKTİF (Bakıma Al)" if shopier_status == "on" else "🛍️ Shopier: BAKIMDA (Aktif Et)"
    manual_btn_text = "💬 Manuel: AKTİF (Bakıma Al)" if manual_status == "on" else "💬 Manuel: BAKIMDA (Aktif Et)"
    
    markup = types.InlineKeyboardMarkup(row_width=1)
    markup.add(
        types.InlineKeyboardButton(crypto_btn_text, callback_data="admin_sys_paytoggle_crypto"),
        types.InlineKeyboardButton(shopier_btn_text, callback_data="admin_sys_paytoggle_shopier"),
        types.InlineKeyboardButton(manual_btn_text, callback_data="admin_sys_paytoggle_manual"),
        types.InlineKeyboardButton("🔙 Sistem Kontrolü", callback_data="admin_system_control")
    )
    return markup
...
def register_admin_handlers(bot):
    ...
    def admin_main_callback_router(call):
        ...
        if data == "admin_menu_fiyatlar":
            ...
            refill_pool = float(database.get_setting("pool_grizzly_refill_tl", "0.0000"))
            net_profit_pool = float(database.get_setting("pool_net_profit_tl", "0.0000"))
            current_rate = float(database.get_setting("last_known_usdt_rate", "46.5000"))
            ...
            metin = (
                ...
                "📊 *FİNANSAL HAVUZ DURUMU*\n"
                "───────────────────\n"
                f"💵 *Canlı Dolar Kuru:* `{current_rate:.4f} TL`\n"
                f"♻️ *Grizzly Yenileme Havuzu:* `{refill_pool:.2f} TL`\n"
                f"└ _(Grizzly'i doldurmak için bankada tutulması gereken bütçe)_\n\n"
                f"💰 *Toplam Net Kârınız:* `{net_profit_pool:.2f} TL`\n"
                f"└ _(Gönül rahatlığıyla harcayabileceğiniz net kazanç)_\n"
                "───────────────────"
            )
            ...
            metin = (
                ...
                f"🆔 ID: `{user['user_id']}`\n"
                f"💳 Bakiye: {user['balance']} TL\n"
                f"⚠️ Uyarı Sayısı: `{user.get('warnings', 0)} / 5`\n"
                f"🛡️ Durum: {drm}\n"
            ...
            markup.row(types.InlineKeyboardButton("➕ Uyarı Ekle", callback_data=f"admin_profwarn_add_{uid}"),
                       types.InlineKeyboardButton("➖ Uyarı Sıfırla", callback_data=f"admin_profwarn_reset_{uid}"))
            ...
            try: bot.answer_callback_query(call.id, "Numara alım durumu güncellendi.")
            except: pass
            ...
            try: bot.answer_callback_query(call.id, "Ödeme yapma durumu güncellendi.")
            except: pass
            call.data = f"admin_usrdet_{uid}"
            admin_main_callback_router(call)
            ...
            security.BANNED_CACHE.discard(uid) # RAM ban önbelleğini temizle
            try: bot.answer_callback_query(call.id, "⚠️ Kullanıcı veritabanından silindi.", show_alert=True)
            except: pass
            ...
            try: bot.answer_callback_query(call.id, "Kullanıcı durumu güncellendi.")
            except: pass
        ...
        elif data == "admin_sys_payment_methods":
            crypto_status = "✅ AKTİF" if database.get_setting("payment_method_crypto", "on") == "on" else "⚠️ BAKIMDA"
            shopier_status = "✅ AKTİF" if database.get_setting("payment_method_shopier", "off") == "on" else "⚠️ BAKIMDA"
            manual_status = "✅ AKTİF" if database.get_setting("payment_method_manual", "on") == "on" else "⚠️ BAKIMDA"
            
            metin = (
                "💳 *Ödeme Yöntemleri Bakım Paneli* 💳\n\n"
                "Her bir ödeme yöntemini bağımsız olarak aktif/pasif edebilirsiniz:\n\n"
                f"⚡ *Kripto Ödemeleri (USDT/TRX):* `{crypto_status}`\n"
                f"🛍️ *Shopier Ödemeleri:* `{shopier_status}`\n"
                f"💬 *Manuel Ödemeler (IBAN/DM):* `{manual_status}`"
            )
            safe_admin_edit(bot, chat_id, msg_id, metin, payment_methods_control_klavyesi())
            bot.answer_callback_query(call.id)
            return

        elif data.startswith("admin_sys_paytoggle_"):
            method = data.replace("admin_sys_paytoggle_", "")
            key = f"payment_method_{method}"
            current = database.get_setting(key, "on" if method != "shopier" else "off")
            new_val = "off" if current == "on" else "on"
            database.set_setting(key, new_val)
            
            bot.answer_callback_query(call.id, f"{method.upper()} durumu güncellendi: {new_val.upper()}", show_alert=True)
            ...
            parts = data.split("_")
            pay_type = parts[1] # "iban" veya "reg"
            uid = int(parts[2])
            mkt = float(parts[3])
            ...
            if pay_type == "iban":
                # Tarihçeye "IBAN (Manuel)" olarak ekle
                database.add_to_history(uid, 5, "Bakiye", "IBAN (Manuel)", -mkt, status="✅ BAŞARILI")
                try: bot.send_message(uid, f"🎉 *Banka transferiniz (IBAN) onaylandı* ve hesabınıza `{mkt} TL` tanımlandı.", parse_mode="Markdown")
                except: pass
                
                # Referans komisyonunu hesapla ve aktar
                from handlers.payment_handler import process_referral_commission
                process_referral_commission(bot, uid, mkt)
                
                info_msg = f"✅ {uid} ID'li hesaba {mkt} TL IBAN ödemesi eklendi ve referans komisyonu tetiklendi."
            else:
                # Tarihçeye "Admin Paneli" olarak ekle
                database.add_to_history(uid, 5, "Bakiye", "Admin Paneli", -mkt, status="✅ BAŞARILI")
                try: bot.send_message(uid, f"🎉 Hesabınıza `{mkt} TL` tanımlandı.", parse_mode="Markdown")
                except: pass
                
                info_msg = f"✅ {uid} ID'li hesaba {mkt} TL düzeltme bakiyesi eklendi (Komisyon tetiklenmedi)."
                
            safe_admin_edit(bot, chat_id, msg_id, info_msg, admin_klavyesi())
            bot.answer_callback_query(call.id, "Bakiye işlemi onaylandı.")
            return
            ...
            markup = types.InlineKeyboardMarkup(row_width=1)
            markup.add(
                types.InlineKeyboardButton("➕ Genel Kupon Oluştur", callback_data="admin_create_coupon_start"),
                types.InlineKeyboardButton("🔒 Tek Seferlik Özel Kupon Oluştur", callback_data="admin_create_priv_coupon_start"),
                types.InlineKeyboardButton("📋 Mevcut Kuponları Listele (Genel)", callback_data="admin_list_coupons"),
                types.InlineKeyboardButton("🔙 Admin Menü", callback_data="admin_main")
            )
            ...
            safe_admin_edit(bot, chat_id, msg_id, "🎟️ *Yeni Genel Kupon Oluşturma Sihirbazı*\n\nFormat: `KOD ÖDÜL LİMİT`\nÖrn: `YENI30 30 100`\n\n✍️ Lütfen oluşturmak istediğiniz genel kupon bilgilerini bu sohbete yazın:", admin_klavyesi())
            bot.clear_step_handler_by_chat_id(chat_id)
            bot.register_next_step_handler(call.message, process_create_coupon, photo_message_id=msg_id)
            try: bot.answer_callback_query(call.id)
            except: pass

        elif data == "admin_create_priv_coupon_start":
            safe_admin_edit(bot, chat_id, msg_id, "🔒 *Tek Seferlik Özel Kupon Oluşturma*\n\nFormat: `KOD ÖDÜL`\nÖrn: `Ali-Agam 100`\n\n✍️ Lütfen oluşturmak istediğiniz özel kupon bilgilerini bu sohbete yazın.\n_(Bot kupon kodunun başına otomatik olarak rastgele benzersiz bir token ekleyecektir. Limit otomatik olarak 1-Tek seferlik olacaktır.)_", admin_klavyesi())
            bot.clear_step_handler_by_chat_id(chat_id)
            bot.register_next_step_handler(call.message, process_create_priv_coupon, photo_message_id=msg_id)
            try: bot.answer_callback_query(call.id)
            except: pass
            ...
            if not coupons:
                ...
                for c in coupons:
                    coupon_link = f"https://t.me/{bot_username}?start=cpn_{c['code']}"
                    # Metin listesine detayları ekle
                    metin += (
                        f"🎟️ *Kod:* `{c['code']}`\n"
                        f"├ 💰 Ödül: `{c['reward_amount']} TL`\n"
                        f"├ 📊 Limit: `{c['used_count']}/{c['usage_limit']}`\n"
                        f"└ 🔗 Link: `{coupon_link}`\n───────────────────\n"
                    )
        ...
        elif data.startswith("admin_warn_add_") or data.startswith("admin_profwarn_add_"):
            from_profile = 1 if data.startswith("admin_profwarn_add_") else 0
            uid = int(data.replace("admin_profwarn_add_", "").replace("admin_warn_add_", ""))
            
            metin = (
                "⚠️ *Kullanıcıya Uyarı Ekleme* ⚠️\n\n"
                f"Kullanıcı ID: `{uid}`\n\n"
                "💬 Lütfen eklemek istediğiniz *uyarı nedenini* sohbete yazın.\n\n"
                "💡 Neden eklemeden doğrudan uyarı göndermek istiyorsanız aşağıdaki *'🚫 Sebepsiz Uyarı Gönder'* butonunu kullanabilirsiniz."
            )
            markup = types.InlineKeyboardMarkup(row_width=1)
            markup.add(
                types.InlineKeyboardButton("🚫 Sebepsiz Uyarı Gönder", callback_data=f"admin_warn_noreas_{uid}_{from_profile}"),
                types.InlineKeyboardButton("🔙 İptal / Geri Dön", callback_data=f"admin_usrdet_{uid}" if from_profile else f"admin_manage_user_{uid}")
            )
            safe_admin_edit(bot, chat_id, msg_id, metin, markup)
            bot.register_next_step_handler(call.message, process_admin_add_warning_reason, photo_message_id=msg_id, uid=uid, from_profile=from_profile)
            bot.answer_callback_query(call.id)
            return

        elif data.startswith("admin_warn_noreas_"):
            parts = data.replace("admin_warn_noreas_", "").split("_")
            uid = int(parts[0])
            from_profile = int(parts[1])
            execute_warning_addition(chat_id, msg_id, uid, "Belirtilmedi", from_profile)
            bot.answer_callback_query(call.id, "Kullanıcıya sebepsiz uyarı gönderildi.")
            return
        ...
        elif data.startswith("admin_profwarn_reset_"):
            uid = int(data.replace("admin_profwarn_reset_", ""))
            database.update_user_warnings(uid, 0)
            try:
                bot.send_message(uid, "✅ *UYARILARINIZ SIFIRLANDI!*\n\nYönetici tarafından uyarı sayacınız sıfırlanmıştır. İşlemlerinize devam edebilirsiniz.", parse_mode="Markdown")
            except:
    ...
    def process_create_coupon(message, photo_message_id):
        ...
        try:
            ...
            # En son 2 parametre: LİMİT ve ÖDÜL. Geriye kalan tüm kelimeler KOD'u oluşturur ve aralarına tire (-) konur.
            limit = int(p[-1])
            odul = float(p[-2])
            kod = "-".join(p[:-2]).upper()
            
            if len(kod) > 50:
                raise ValueError("Kupon kodu (tireler dahil) en fazla 50 karakter uzunluğunda olabilir!")
            
            if database.create_coupon(kod, odul, limit, is_private=0):
                try:
                    bot_username = bot.get_me().username
                except:
                    bot_username = "bot"
                coupon_link = f"https://t.me/{bot_username}?start=cpn_{kod}"
                
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton("🔙 Kupon Menüsüne Dön", callback_data="admin_coupon_menu"))
                safe_admin_edit(bot, message.chat.id, photo_message_id, f"✅ Kupon Başarıyla Oluşturuldu: `{kod}`\n\n🔗 *Paylaşım Linki:*\n`{coupon_link}`", markup)
        except Exception as e: 
            err_msg = str(e) if "karakter" in str(e) else "Hata! Format: `KOD ÖDÜL LİMİT` (Örn: `WELCOME 50 100` -> `WELCOME-50`)\nNot: Kupon ismi en fazla 50 karakter olmalıdır."
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("🔙 Kupon Menüsüne Dön", callback_data="admin_coupon_menu"))
            safe_admin_edit(bot, message.chat.id, photo_message_id, f"❌ {err_msg}", markup)

    def process_create_priv_coupon(message, photo_message_id):
        if not is_admin(message.from_user.id): return
        bot.clear_step_handler_by_chat_id(message.chat.id); cleanup_msg(message)
        try:
            import random
            import string
            p = message.text.split()
            # En son parametre ÖDÜL. Geriye kalan tüm kelimeler KOD'u oluşturur ve aralarına tire (-) konur.
            odul = float(p[-1])
            kod_base = "-".join(p[:-1]).upper()
            
            # 6 Haneli Rastgele Token Üret
            token = "".join(random.choices(string.ascii_uppercase + string.digits, k=6))
            full_kod = f"{token}_{kod_base}"
            
            if len(full_kod) > 50:
                raise ValueError("Kupon ismi ve otomatik eklenen token toplamı en fazla 50 karakter olabilir!")
            
            # Veritabanına is_private=1 ve usage_limit=1 olarak kaydet
            if database.create_coupon(full_kod, odul, 1, is_private=1):
                try:
                    bot_username = bot.get_me().username
                except:
                    bot_username = "bot"
                coupon_link = f"https://t.me/{bot_username}?start=cpn_{full_kod}"
                
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton("🔙 Kupon Menüsüne Dön", callback_data="admin_coupon_menu"))
                safe_admin_edit(bot, message.chat.id, photo_message_id, f"✅ *Tek Seferlik Özel Kupon Oluşturuldu!*\n\n🎫 Kod: `{full_kod}`\n💰 Ödül: `{odul} TL`\n\n🔗 *Özel Paylaşım Linki:*\n`{coupon_link}`", markup)
        except Exception as e: 
            err_msg = str(e) if "karakter" in str(e) else "Hata! Format: `KOD ÖDÜL` (Örn: `Ahmet Agam 100` -> `AHMET-AGAM`)\nNot: Kupon ismi en fazla 40 karakter olmalıdır."
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("🔙 Kupon Menüsüne Dön", callback_data="admin_coupon_menu"))
            safe_admin_edit(bot, message.chat.id, photo_message_id, f"❌ {err_msg}", markup)
    ...
    def process_add_balance(message, photo_message_id):
        ...
        try:
            p = message.text.split()
            uid, mkt = int(p[0]), float(p[1])
            
            # Onay ekranı göster
            metin = (
                "💳 *Manuel Bakiye Yükleme Onayı* 💳\n\n"
                f"👤 *Kullanıcı ID:* `{uid}`\n"
                f"💵 *Miktar:* `{mkt} TL`\n\n"
                "Lütfen bu işlemin türünü seçiniz. IBAN transferi seçilirse kullanıcının davet edenine (varsa ve onaylıysa) referans komisyonu otomatik olarak yansıtılacaktır:"
            )
            markup = types.InlineKeyboardMarkup(row_width=1)
            markup.add(
                types.InlineKeyboardButton("🏦 Evet (IBAN Transferi)", callback_data=f"adbal_iban_{uid}_{mkt}"),
                types.InlineKeyboardButton("⚙️ Hayır (Düzeltme / Diğer)", callback_data=f"adbal_reg_{uid}_{mkt}"),
                types.InlineKeyboardButton("🔙 İptal Et", callback_data="admin_main")
            )
            safe_admin_edit(bot, message.chat.id, photo_message_id, metin, markup)
        except:
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("🔙 Tekrar Dene", callback_data="admin_add_balance"))
            safe_admin_edit(bot, message.chat.id, photo_message_id, "❌ Hata! Format: `USER_ID MİKTAR` (Örn: `123456789 100`)", markup)
    ...
    def process_broadcast(message, photo_message_id):
        ...
        # Admin'e anında bildirim gönder
        safe_admin_edit(bot, chat_id, photo_message_id, f"📢 Duyuru gönderimi arka planda başlatıldı!\n\n👥 Hedef Kitle: `{len(users)}` kullanıcı.", admin_klavyesi())
        ...
        def broadcast_worker():
            ...
            for u in users:
                try:
                    bot.send_message(u['user_id'], f"📢 *Duyuru*\n\n{duyuru_metni}", parse_mode="Markdown")
                    basarili += 1
                    time.sleep(0.04) # Saniyede max 25 mesaj (Telegram Sınırı: 30)
                except Exception as e:
                    hatali += 1
                    if "Too Many Requests" in str(e) or "retry after" in str(e):
                        try:
                            # Rate limit yendiyse 3 saniye bekle ve tekrar dene
                            time.sleep(3.0)
                            bot.send_message(u['user_id'], f"📢 *Duyuru*\n\n{duyuru_metni}", parse_mode="Markdown")
                            basarili += 1
                        except:
            ...
            # Admin'e sonuç raporu gönder
            try:
                bot.send_message(chat_id, f"📢 *Duyuru Gönderimi Tamamlandı!*\n\n✅ Başarılı: `{basarili}`\n❌ Başarısız/Engellemiş: `{hatali}`\n👥 Toplam: `{len(users)}`", parse_mode="Markdown")
            except:
                pass
    ...
    def process_admin_add_warning_reason(message, photo_message_id, uid, from_profile):
        if not is_admin(message.from_user.id): return
        bot.clear_step_handler_by_chat_id(message.chat.id); cleanup_msg(message)
        reason = message.text.strip() if message.text else "Belirtilmedi"
        execute_warning_addition(message.chat.id, photo_message_id, uid, reason, from_profile)

    def execute_warning_addition(chat_id, photo_message_id, uid, reason, from_profile):
        warnings = database.get_user_warnings(uid)
        if warnings < 5:
            warnings += 1
            database.update_user_warnings(uid, warnings)
            if warnings >= 5:
                database.ban_user(uid, 1, f"Limit Aşımı (Neden: {reason})", f"Manuel Uyarı Limiti ({reason})")
                database.update_user_warnings(uid, 0)
                from providers.security_manager import security
                security.BANNED_CACHE.add(uid)
                try:
                    bot.send_message(uid, f"🚫 *UYARI SINIRI AŞILDI!*\n\n5 adet uyarı aldığınız için hesabınız sistem tarafından süresiz olarak yasaklanmıştır.\n└ 📝 Son Uyarı Nedeni: `{reason}`", parse_mode="Markdown")
                except:
                    pass
            else:
                try:
                    bot.send_message(uid, f"⚠️ *UYARI ALDINIZ!*\n\nYönetici tarafından uyarıldınız.\n\n*Mevcut Uyarı:* `{warnings} / 5`\n└ 📝 Uyarı Nedeni: `{reason}`\n\n_Lütfen kurallara uyunuz. 5. uyarıda hesabınız yasaklanacaktır!_", parse_mode="Markdown")
                except:
                    pass
        ...
        if from_profile:
            ...
            show_user_ban_management_panel(bot, chat_id, photo_message_id, uid)
