        ...
        if input_type == "coupon":
            if not re.match(r"^[A-Z0-9_.-]+$", text_str.upper()):
                if is_malicious or "'" in text_str or ";" in text_str or '"' in text_str:
                    ...
                    self.BANNED_CACHE.add(user_id)
                    return False, "🚫 *GÜVENLİK İHLALİ!* Sisteme yönelik manipülasyon girişimi tespit edildi. Sistem kurallarını ihlal ettiğiniz için hesabınız **KALICI OLARAK BANLANDI!**", {"type": 3, "reason": "XSS/SQLi Saldırısı", "input": text_str}
