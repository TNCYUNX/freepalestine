...
def get_active_connections():
    if not hasattr(_thread_local, "active_conns"):
        _thread_local.active_conns = []
    return _thread_local.active_conns
...
class SafeConnectionWrapper:
    def __init__(self, conn):
        self._conn = conn
        self._closed = False
        get_active_connections().append(self)
    ...
    def close(self):
        if not self._closed:
            ...
            self._closed = True
            try:
                get_active_connections().remove(self)
            except ValueError:
                pass
...
def normalize_phone_number(phone: str) -> str:
    """Telefon numarasını siber güvenlik kurallarına göre normalize eder (+905551234567 formatı)."""
    if not phone:
        return ""
    # Sadece rakamları ayıkla
    digits = re.sub(r"\D", "", phone)

    # Çift sıfırla başlıyorsa uluslararası koddur, sıfırları at
    if digits.startswith("00"):
        digits = digits[2:]
    # Türkiye içi sıfırla başlayan 11 haneli numara ise '90' ile değiştir (0532... -> 90532...)
    elif digits.startswith("0") and len(digits) == 11:
        digits = "90" + digits[1:]
    # Ülke kodsuz doğrudan 5xx ile başlıyorsa Türkiye varsay (532... -> 90532...)
    ...
    try:
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS users (
                ...
                ref_token VARCHAR(50) UNIQUE DEFAULT NULL
            )
        """)
        try:
        ...
        try:
            cursor.execute("ALTER TABLE users ADD COLUMN ref_token VARCHAR(50) UNIQUE DEFAULT NULL")
            conn.commit()
        except Exception:
            pass
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS history (
                ...
                price FLOAT,
                cost_tl DECIMAL(10,4) DEFAULT 0.0000,
                profit_tl DECIMAL(10,4) DEFAULT 0.0000,
        ...
        """)
        try:
            cursor.execute("ALTER TABLE history ADD COLUMN cost_tl DECIMAL(10,4) DEFAULT 0.0000")
            conn.commit()
        except Exception:
            pass
        try:
            cursor.execute("ALTER TABLE history ADD COLUMN profit_tl DECIMAL(10,4) DEFAULT 0.0000")
            conn.commit()
        except Exception:
            pass
        try:
        ...
        """)
        try: cursor.execute("ALTER TABLE coupons ADD COLUMN is_private INT DEFAULT 0")
        except: pass
        try: cursor.execute("ALTER TABLE coupons MODIFY COLUMN code VARCHAR(150) UNIQUE")
        except: pass
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS services (
                ...
                api_max_price DECIMAL(10,4) DEFAULT NULL,
        ...
        cursor.execute("INSERT IGNORE INTO settings (key_name, key_value) VALUES ('payment_method_crypto', 'on')")
        cursor.execute("INSERT IGNORE INTO settings (key_name, key_value) VALUES ('payment_method_shopier', 'off')")
        cursor.execute("INSERT IGNORE INTO settings (key_name, key_value) VALUES ('payment_method_manual', 'on')")
        cursor.execute("INSERT IGNORE INTO settings (key_name, key_value) VALUES ('pool_grizzly_refill_tl', '0.0000')")
        cursor.execute("INSERT IGNORE INTO settings (key_name, key_value) VALUES ('pool_net_profit_tl', '0.0000')")
        cursor.execute("INSERT IGNORE INTO settings (key_name, key_value) VALUES ('last_known_usdt_rate', '46.5000')")
...
def add_user(user_id, username, initial_balance=0.0, referred_by=None):
    if referred_by:
        ...
        if not ref_info or ref_info.get("ref_status") != "approved":
            referred_by = None
    ...
    # Benzersiz 16 karakterli referans kodu (token) üret
    import secrets
    import string
    ...
    cursor.execute(
        "INSERT INTO users (user_id, username, balance, referred_by, ref_token) VALUES (%s, %s, %s, %s, %s)",
        (user_id, username, initial_balance, referred_by, ref_token)
    )
    conn.commit()
    conn.close()
...
def get_all_users():
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT user_id, username, balance, is_banned, ban_type, ban_reason, banned_input, referred_by, phone_number, ref_status, number_buy_blocked, deposit_blocked, warnings FROM users")
    results = cursor.fetchall()
    conn.close()
    return results
...
def ban_user(user_id, ban_type=0, reason=None, input_data=None):
    ...
    if admin_id and str(user_id) == str(admin_id):
        print(f"[!] GÜVENLİK UYARISI: Admin ({user_id}) banlanmaya çalışıldı, işlem engellendi.")
        return
    ...
    if user_exists:
        # Eğer zaten varsa ban durumunu güncelle
        ...
        # Eğer yoksa sıfırdan banlı olarak veritabanına ekle
        cursor.execute("INSERT INTO users (user_id, username, is_banned, ban_type, ban_reason, banned_input) VALUES (%s, 'Spammer', TRUE, %s, %s, %s)", (user_id, ban_type, reason, input_data))
...
def update_service_cost_by_id(service_id, new_cost):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE services SET api_max_price = %s WHERE id = %s", (new_cost, service_id))
    conn.commit()
    conn.close()
...
def get_setting(key_name, default_value="on"):
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
...
def create_coupon(code, reward_amount, usage_limit, is_private=0):
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO coupons (code, reward_amount, usage_limit, is_private) VALUES (%s, %s, %s, %s)", (code, reward_amount, usage_limit, is_private))
        conn.commit()
        return True
    except: return False
    finally: conn.close()
...
def redeem_coupon(user_id, code):
    ...
    try:
        ...
        is_single_use = (coupon.get('is_private') == 1 or coupon['usage_limit'] == 1)
        if is_single_use:
            # Tek kullanımlık veya özel kupon, kullanıldıktan sonra yer kaplamaması için siliniyor
            cursor.execute("DELETE FROM coupons WHERE id = %s", (coupon['id'],))
            cursor.execute("DELETE FROM used_coupons WHERE coupon_id = %s", (coupon['id'],))
        else:
...
def get_all_coupons():
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT code, reward_amount, usage_limit, used_count FROM coupons WHERE is_private = 0 ORDER BY id DESC")
    results = cursor.fetchall()
    conn.close()
    return results
...
def add_referral(referrer_id, referred_id):
    ...
    ref_info = get_user_info(referrer_id)
    if not ref_info or ref_info.get("ref_status") != "approved":
        print(f"[REJECT add_referral] Referrer {referrer_id} is not approved!")
        return False
...
def verify_user_phone(user_id, phone):
    ...
    status = "approved" if phone.startswith("+90") else "pending"
...
def get_user_info(user_id):
    ...
    try:
        cursor = conn.cursor(dictionary=True)
        cursor.execute("SELECT user_id, username, balance, is_banned, referred_by, phone_number, ref_status, number_buy_blocked, deposit_blocked, ref_token FROM users WHERE user_id = %s", (user_id,))
...
def get_user_by_ref_token(ref_token):
    """Kullanıcının referans koduna göre bilgilerini veritabanından çeker."""
    conn = get_db_connection()
    if not conn: return None
    try:
        cursor = conn.cursor(dictionary=True)
        cursor.execute("SELECT user_id, username, balance, is_banned, referred_by, phone_number, ref_status, number_buy_blocked, deposit_blocked, ref_token FROM users WHERE ref_token = %s", (ref_token,))
        return cursor.fetchone()
    except Exception as e:
        print(f"Error in get_user_by_ref_token: {e}")
        return None
    finally:
        conn.close()
...
def toggle_user_number_buy_blocked(user_id):
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("UPDATE users SET number_buy_blocked = 1 - COALESCE(number_buy_blocked, 0) WHERE user_id = %s", (user_id,))
        conn.commit()
        return True
    except Exception as e:
...
def toggle_user_deposit_blocked(user_id):
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("UPDATE users SET deposit_blocked = 1 - COALESCE(deposit_blocked, 0) WHERE user_id = %s", (user_id,))
        conn.commit()
        return True
    except Exception as e:
...
def get_live_usdt_rate():
    """Binance üzerinden anlık USDTTRY kurunu çeker. Hata durumunda son başarılı kuru döndürür."""
    import requests
    try:
        r = requests.get("https://api.binance.com/api/v3/ticker/price?symbol=USDTTRY", timeout=3)
        if r.status_code == 200:
            current_rate = float(r.json().get("price"))
            # Son başarılı kuru veritabanına yedekle
            update_setting("last_known_usdt_rate", str(current_rate))
            return current_rate
    except Exception as e:
        print(f"[-] Kur çekme hatası (Yedek kur kullanılıyor): {e}")

    # API hata verirse veritabanındaki son başarılı kuru çek, o da yoksa 46.50 döndür
    return float(get_setting("last_known_usdt_rate", "46.50"))

def add_to_history_with_profit(user_id, action_type, service_name, fake_number, price, cost_tl, profit_tl, service_code=None, activation_id=None):
    """Maliyet ve kâr detaylarıyla birlikte işlem geçmişini veritabanına yazar."""
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO history (user_id, action_type, service_name, fake_number, price, cost_tl, profit_tl, status, service_code, activation_id) "
        "VALUES (%s, %s, %s, %s, %s, %s, %s, '✅ BAŞARILI', %s, %s)",
        (user_id, action_type, service_name, fake_number, price, cost_tl, profit_tl, service_code, activation_id)
    )
    conn.commit()
    conn.close()

def update_financial_pools(cost_tl, profit_tl):
    """Maliyet ve kâr tutarlarını biriken havuz sayaçlarına ekler (Atomik Güncelleme)."""
    conn = get_db_connection()
    cursor = conn.cursor()

    # Grizzly Yenileme Bütçe Havuzunu Güncelle
    cursor.execute(
        "UPDATE settings SET key_value = CAST(CAST(key_value AS DECIMAL(15,4)) + %s AS CHAR) WHERE key_name = 'pool_grizzly_refill_tl'",
        (cost_tl,)
    )

    # Net Kâr Havuzunu Güncelle
    cursor.execute(
        "UPDATE settings SET key_value = CAST(CAST(key_value AS DECIMAL(15,4)) + %s AS CHAR) WHERE key_name = 'pool_net_profit_tl'",
        (profit_tl,)
    )
...
def db_safe_decorator(func):
    """Veritabanı CRUD fonksiyonlarında olası exception fırlamalarında bağlantıların
    her koşulda ve anında havuza iade edilmesini garanti eden dinamik zırh."""
    from functools import wraps
    @wraps(func)
    def wrapper(*args, **kwargs):
        conns_before = list(get_active_connections())
        try:
            return func(*args, **kwargs)
        finally:
            conns_after = list(get_active_connections())
            for conn in conns_after:
                if conn not in conns_before:
                    try:
                        conn.close()
                    except Exception as e:
                        print(f"[-] Auto-close connection error in database function {func.__name__}: {e}")
    return wrapper

# Modüldeki tüm CRUD fonksiyonlarını otomatik olarak bu sarmalayıcı ile zırhlandır
import inspect
import sys
...
for name, obj in list(globals().items()):
    if (inspect.isfunction(obj) and 
        ...
        name not in ('get_db_connection', 'db_safe_decorator', 'get_active_connections', 'setup_database')):
        globals()[name] = db_safe_decorator(obj)
