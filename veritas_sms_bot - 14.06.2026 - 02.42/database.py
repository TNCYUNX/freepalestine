...
def setup_database():
    ...
    try:
        ...
        # İstediğin Popüler Ülkeleri Zirveye Taşı
        populer_ulkeler = {
...
def safe_decrease_balance(user_id, amount):
    """Race Condition önlemek için bakiyeyi atomik olarak düşer."""
    conn = get_db_connection()
    cursor = conn.cursor()
...
def is_tx_processed(txid):
    """Kripto işleminin daha önce kullanılıp kullanılmadığını kontrol eder."""
    conn = get_db_connection()
    cursor = conn.cursor()
...
def mark_tx_processed(txid):
    """Kripto işlemini kullanıldı olarak işaretler."""
    conn = get_db_connection()
    cursor = conn.cursor()
...
def add_user(user_id, username, initial_balance=50.0):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
...
def add_to_history(user_id, service_name, fake_number, price):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO history (user_id, service_name, fake_number, price) VALUES (%s, %s, %s, %s)",
        (user_id, service_name, fake_number, price)
    )
    conn.commit()
    conn.close()

def add_new_service(srv_name, api_srv, cc_code, c_name, api_cc, price, flag, api_max_price=None):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO services (service_name, api_srv, country_code, country_name, api_cc, price, flag, api_max_price) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
...
def get_user_payment_history(user_id, limit=5):
    return get_user_history(user_id, limit)

def get_user_canceled_history(user_id, limit=5):
    return []
...
def close_user_tickets(user_id):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE support_tickets SET status = 'kapali' WHERE user_id = %s", (user_id,))
    conn.commit()
    conn.close()
...
def get_all_services():
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT id, service_name, country_name FROM services")
    results = cursor.fetchall()
    conn.close()
    return results
...
def get_countries_for_service(service_name):
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT id, country_code, country_name, price, api_srv, api_cc, flag, api_max_price FROM services WHERE service_name = %s AND is_active = TRUE", (service_name,))
    results = cursor.fetchall()
    conn.close()
    return results
...
def get_service_by_id(service_id):
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT id, service_name, api_srv, country_code, country_name, api_cc, price, is_active, api_max_price, flag FROM services WHERE id = %s", (service_id,))
    res = cursor.fetchone()
    conn.close()
    return res
