...
MESAJLAR = {
    ...
    "gecersiz_miktar": "❌ *Geçersiz miktar.* Lütfen sınırları kontrol edin (Min: {min_limit}, Max: 9999).",
    "fatura_arayuzu": (
        "💳 *Ödeme Bilgileri*\n\n"
        "💰 *Yüklemek istediğiniz:* `{base_amount}` {kur_tipi}\n"
        "🔢 *GÖNDERMENİZ GEREKEN NET MİKTAR:* `{final_amount}` {kur_tipi}\n"
        "📥 *Cüzdan:* `{cuzdan}`\n"
        "🌐 *Ağ:* TRC20 (Tron Mainnet)\n"
        "💵 *Alacağınız bakiye:* `{tl_miktari}` TL\n"
        "📊 *Canlı Kur:* 1 USDT = `{kur_usdt}` TL | 1 TRX = `{kur_trx}` TL\n\n"
        "⚠️ *ÖNEMLİ UYARI:* Borsalardan (Binance, Paribu vb.) gönderim yaparken, borsanın kestiği transfer komisyonunu hesaba katarak, cüzdanımıza *TAM OLARAK* `{final_amount}` {kur_tipi} geçecek şekilde çekim talebi veriniz!\n"
        "Eksik veya farklı miktar gönderirseniz ödeme otomatik onaylanmaz.\n\n"
    ...
    "qr_mesaji": (
        ...
        "📥 *Cüzdan:* `{cuzdan}`\n\n"
        "*(Okutarak doğrudan cüzdan adresini kopyalayabilirsiniz)*"
    ),
