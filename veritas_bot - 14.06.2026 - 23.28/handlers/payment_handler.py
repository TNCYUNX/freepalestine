...
def safe_payment_edit(bot, chat_id, message_id, text, markup=None):
    """Ödeme mesajlarını caption veya text olarak güvenli bir şekilde günceller."""
    try:
...
def register_payment_handlers(bot):
    """Bakiye yükleme, QR gösterimi ve otomatik ödeme tarama işlemlerini yöneten modül."""
    ...
    def payment_callback_router(call):
        ...
        # 1. Menü: Bakiye Yükle (Birim Seçimi)
            ...
            for tx in transfers:
                if tx["type"] == kur_tipi and abs(tx["amount"] - float(miktar)) < 0.001:
                    if database.atomic_mark_tx_processed(tx["txid"]):
                        ...
                        database.add_to_history(user_id, "Bakiye", "Manuel Onay", -tl_eklenen, status="✅ BAŞARILI")
                        
                        # Güncel toplam bakiyeyi çek
                        yeni_bakiye = database.get_balance(user_id)
                        
                        # EKSİKSİZ BAŞARI RAPORU
                        basari_metni = (
                            "✅ *ÖDEME BAŞARILI!*\n\n"
                        ...
                        bot.answer_callback_query(call.id, "✅ Ödeme Onaylandı!")
                        found = True
                        break
            
            if not found:
                # NET VE DOĞRUDAN SONUÇ
                bot.answer_callback_query(call.id, "❌ Henüz ödeme tamamlanmadı veya ağda bulunamadı. Lütfen 1-2 dakika sonra tekrar deneyin.", show_alert=True)
...
def process_bakiye_miktari_al(message, bot, photo_message_id, kur_tipi):
    ...
    try:
        ...
        with odeme_lock:
            BEKLEYEN_ODEMELER[f"{kur_tipi}_{final_amount}"] = user_id
