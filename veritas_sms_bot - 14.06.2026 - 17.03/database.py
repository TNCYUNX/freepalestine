...
def add_to_history(user_id, service_name, fake_number, price, status="✅ BAŞARILI"):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO history (user_id, service_name, fake_number, price, status) VALUES (%s, %s, %s, %s, %s)",
        (user_id, service_name, fake_number, price, status)
    )
    conn.commit()
    conn.close()
