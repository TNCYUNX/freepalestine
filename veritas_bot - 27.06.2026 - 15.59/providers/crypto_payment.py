...
def check_bsc_usdt_transfer(beklenen_tutar, min_time_ms=0):
    """
    BNB Smart Chain (BEP20) üzerinden USDT (0x55d398326f99059fF775485246999027B3197955) ödemelerini doğrular.
    """
    import database
    api_key = os.getenv("BSCSCAN_API_KEY", "").strip()
    wallet = os.getenv("BSC_WALLET_ADDRESS", "").strip()
    if not api_key or not wallet:
        return False, 0.0

    usdt_kontrat_adresi = "0x55d398326f99059fF775485246999027B3197955"
    url = f"https://api.bscscan.com/api?module=account&action=tokentx&contractaddress={usdt_kontrat_adresi}&address={wallet}&page=1&offset=20&startblock=0&endblock=999999999&sort=desc&apikey={api_key}"
        ...
        if res.status_code == 200:
            ...
            if data.get("status") == "1" and data.get("message") == "OK":
                for tx in data.get("result", []):
                    tx_time_ms = int(tx.get("timeStamp", 0)) * 1000
                    
                    # Fatura oluşturulma zamanından önceki işlemleri atla
                    if tx_time_ms < min_time_ms:
                        continue

                    # Sadece bize gelen transferleri kontrol et
                    if tx.get("to", "").lower() == wallet.lower():
                        value_wei = float(tx.get("value", "0"))
                        amount = value_wei / (10**18)
                        
                        if abs(amount - float(beklenen_tutar)) < 0.001:
                            txid = tx.get("hash")
                            if txid and not database.is_tx_processed(txid):
                                if database.atomic_mark_tx_processed(txid):
                                    return True, amount
    except Exception as e:
        print(f"BscScan API Hatası: {e}")
