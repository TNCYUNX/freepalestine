...
def get_db_connection(database=True):
    ...
    try:
        if database:
            ...
            return SafeConnectionWrapper(raw_conn) if raw_conn else None
        else:
...
def get_all_users():
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT user_id, username, balance, is_banned, ban_type, ban_reason, banned_input, referred_by, phone_number, ref_status FROM users")
    results = cursor.fetchall()
    conn.close()
    return results
...
def get_user_info(user_id):
    ...
    try:
        cursor = conn.cursor(dictionary=True)
        cursor.execute("SELECT user_id, username, balance, is_banned, referred_by, phone_number, ref_status FROM users WHERE user_id = %s", (user_id,))
