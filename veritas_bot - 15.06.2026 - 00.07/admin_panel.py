...
def register_admin_handlers(bot):
    ...
    def admin_main_callback_router(call):
        ...
        if data == "admin_main":
            ...
            if act == "1": database.ban_user(uid)
            else: database.unban_user(uid)
            ...
            try:
                ...
                cursor.execute("SELECT service_name, fake_number, price, date, status FROM history WHERE user_id = %s ORDER BY id DESC LIMIT 20", (uid,))
    ...
    def process_create_coupon(message, photo_message_id):
        ...
        try:
            ...
            if database.create_coupon(kod, odul, limit): safe_admin_edit(bot, message.chat.id, photo_message_id, f"✅ Kupon Oluşturuldu: `{kod}`", admin_klavyesi())
        except: safe_admin_edit(bot, message.chat.id, photo_message_id, "❌ Hata! Format: `KOD ÖDÜL LİMİT`", admin_klavyesi())
