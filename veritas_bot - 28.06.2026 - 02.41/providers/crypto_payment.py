...
if TRON_NETWORK == "mainnet":
    ...
    USDT_CONTRACT = "TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t" # Resmi USDT (TRC20) Kontrat Adresi
else:
    BASE_URL = "https://nile.trongrid.io"
    NETWORK_NAME = "Tron Nile Testnet"
    USDT_CONTRACT = "TXYZopSpLH217cKZW9CzP12ycYcGPzJAe3" # Nile Testnet USDT Kontrat Adresi
...
def get_valid_incoming_transfers(min_time_ms=0):
    ...
    cuzdan_hex = base58_to_hex(cuzdan)
    ...
    try:
        ...
        if res.status_code == 200:
            for tx in res.json().get("data", []):
                ...
                if tx.get("to") == cuzdan and tx.get("token_info", {}).get("address", "") == USDT_CONTRACT:
            ...
            for tx in res.json().get("data", []):
                ...
                if contracts and contracts[0].get("type") == "TransferContract":
                    ...
                    if ret and ret[0].get("contractRet") == "SUCCESS":
                        val = contracts[0].get("parameter", {}).get("value", {})
                        to_address = val.get("to_address", "")
                        
                        # Alıcı adresinin bizim cüzdan hex adresiyle uyuştuğunu kesinleştir
                        if cuzdan_hex and to_address == cuzdan_hex:
                            amount_sun = float(val.get("amount", 0))
