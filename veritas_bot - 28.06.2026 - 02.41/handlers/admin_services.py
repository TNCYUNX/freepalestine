...
def register_admin_service_handlers(bot):
    ...
    @bot.callback_query_handler(func=lambda call: call.data.startswith(('admin_menu_fiyatlar', 'admin_add_srv_', 'admin_adds_', 'admin_pagnat_', 'admin_del_price_start', 'admin_del_srv_', 'admin_del_id_', 'admin_edit_price_start', 'admin_edit_srv_', 'admin_edit_cc_', 'admin_edit_id_', 'admin_compare_start', 'admin_comp_')))
    def admin_service_callback_router(call):
        ...
        # --- SERVİS SİLME ---
            ...
            conn = database.get_db_connection()
            cursor = conn.cursor(dictionary=True)
            cursor.execute("SELECT id, country_name, flag FROM services WHERE api_srv = %s ORDER BY country_name", (api_srv,))
            countries = cursor.fetchall()
            conn.close()
            
            markup = types.InlineKeyboardMarkup(row_width=2)
            for c in countries:
                markup.add(types.InlineKeyboardButton(f"{c['flag']} {c['country_name']}", callback_data=f"admin_del_id_{c['id']}"))
            ...
            if api_srv: call.data = f"admin_del_srv_{api_srv}"; admin_service_callback_router(call)
            else: call.data = "admin_del_price_start"; admin_service_callback_router(call)
        ...
        # --- FİYAT GÜNCELLEME (Sağlayıcıya Duyarlı) ---
            ...
            conn = database.get_db_connection()
            cursor = conn.cursor(dictionary=True)
            cursor.execute("SELECT DISTINCT country_code, country_name, flag FROM services WHERE api_srv = %s ORDER BY country_name", (api_srv,))
            countries = cursor.fetchall()
            conn.close()
            ...
            for c in packages:
                ...
                btn_text = f"💰 {c['price']} TL | 💵 Alış: {c['api_max_price']} $ | 📦 Stok: {stok}"
            ...
            metin += "✍️ Lütfen yeni **TL Satış Fiyatını** yazın (Örn: 15.5):"

            
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("🔙 Geri", callback_data=f"admin_edit_cc_{svc['api_srv']}_{svc['country_code']}"))
            safe_admin_edit(bot, chat_id, msg_id, metin, markup)
...
def process_maliyet_yakala(message, bot, api_srv, api_cc, msg_id):
    ...
    try:
        ...
        bot.send_message(message.chat.id, "❌ Hata: Geçersiz maliyet. Lütfen listedeki rakamı (Örn: 0.88) kopyalayıp yapıştırın.")
...
def process_final_tl_kaydet(message, bot, api_srv, api_cc, api_cost, msg_id):
    ...
    try:
        ...
        bot.send_message(message.chat.id, "❌ Hata: Lütfen sadece TL cinsinden rakam giriniz (Örn: 25.5).")
        bot.register_next_step_handler(message, process_final_tl_kaydet, bot=bot, api_srv=api_srv, api_cc=api_cc, api_cost=api_cost, msg_id=msg_id)
...
def process_wizard_edit_price(message, bot, service_id, photo_message_id):
    ...
    try:
        ...
        cleanup_msg(bot, message)
        bot.send_message(message.chat.id, "❌ Hata: Geçersiz fiyat formatı. Lütfen geçerli bir fiyat yazın (Örn: 25.50):")
