        ...
        if time.time() - last_query_time >= 4.0:
            ...
            try:
                ...
                if sonuc == "STATUS_CANCEL":
                    ...
                    database.add_to_history(user_id, 2, svc_name.capitalize(), ulke_numara_kod, fiyat, status="✅ BAŞARILI", service_code=api_srv, activation_id=act_id)
                    database.delete_active_rental(act_id)
...
def recover_active_rentals(bot):
    ...
    for r in rentals:
        ...
        if isinstance(sms_code, str) and sms_code != "WAIT_CODE":
            ...
            database.add_to_history(user_id, 2, svc_name.capitalize(), ulke_numara_kod, price, status="✅ BAŞARILI", service_code=api_srv, activation_id=act_id)
            database.delete_active_rental(act_id)
...
def start_central_sms_tracking_worker(bot):
    """Sistemin tüm aktif SMS kiralamalarını tek bir arka plan iş parçacığı (worker thread)
    ile yöneten, thread tıkanmalarını ve DDoS kilitlenmelerini önleyen merkezi sorgulama motoru."""
    from main import ana_menu_klavyesi
    from providers import grizzly_provider, tigersms_provider
    
    def worker():
        print("[+] Merkezi SMS Takip İşçisi (Worker Thread) başlatıldı.")
        while True:
            try:
                # Eşzamanlılığı korumak için aktif takip anahtarlarını kilit altında kopyala
                with tracking_lock:
                    active_ids = [k for k, v in AKTIF_TAKIPLER.items() if v.get("status")]
                
                now = time.time()
                
                for act_id in active_ids:
                    with tracking_lock:
                        info = AKTIF_TAKIPLER.get(act_id)
                    ...
                    if now - last_ui_update >= 10.0:
                        ...
                        try:
                            ...
                            # Mesaj silindiyse tekrar gönderip message_id'yi güncelle (Siber Kalkan)
                            if "message to edit not found" in str(e).lower() or "message id invalid" in str(e).lower():
                                try:
                                    with open("veritas_sms_logo_yatay.png", "rb") as photo:
                                        yeni_mesaj = bot.send_photo(chat_id, photo, caption=metin, reply_markup=markup, parse_mode="Markdown")
                ...
                print(f"[-] Central worker main loop error: {e}")
            time.sleep(1.0)
