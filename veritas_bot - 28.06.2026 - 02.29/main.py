    ...
    if contact.user_id != user_id:
        ...
        if new_warnings >= 3:
            ...
            user_msg = (
                ...
                f"Sistem uyarı limitinizi (`{new_warnings}/3`) aştığınız için **KALICI OLARAK BANLANDINIZ!**"
            )
            ...
            if admin_id:
                admin_msg = (
                    ...
                    f"📊 *Ceza:* +2 Uyarı verildi. Toplam Uyarı: `{new_warnings}/3` -> **KALICI BAN!**"
                )
            ...
            # 2 Uyarı ver
            user_msg = (
                f"⚠️ *UYARI! ({new_warnings}/3) — Hileli Referans Girişimi!*\n\n"
                "Başka bir kullanıcıya ait telefon numarası kartı paylaştığınız tespit edildi.\n\n"
            ...
            if admin_id:
                admin_msg = (
                    ...
                    f"📊 *Ceza:* +2 Uyarı verildi. Toplam Uyarı: `{new_warnings}/3` (Kritik Eşik)"
                )
                try: bot.send_message(admin_id, admin_msg, parse_mode="Markdown")
                except: pass
        return
    ...
    if data == "back_to_main":
        ...
        try:
            ...
            for idx, op in enumerate(islemler, 1):
                ...
                service_name_val = op.get('service_name') or "Bilinmeyen"
                islem_adi = config.ISLEM_TIPLERI.get(islem_kodu) or service_name_val
            ...
            print(f"[ERROR] menu_gecmisim exception: {e}")
            import traceback
            traceback.print_exc()
            safe_edit(call, f"❌ İşlem geçmişi yüklenirken bir hata oluştu: {e}", geri_don())
        finally:
            if conn:
                try: conn.close()
                except: pass
    ...
    while True:
        try:
            ...
            if current_time - last_backup_time >= 43200:
                ...
                # Run subprocess securely without shell=True, redirecting output directly to file
