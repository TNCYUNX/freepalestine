            ...
            if database.safe_decrease_balance(user_id, svc['price']):
                ...
                if isinstance(sonuc, str):
                    ...
                    database.add_active_rental(
                        ...
                        price=svc['price'],
                        provider=prov,
                        start_time=AKTIF_TAKIPLER[act_id]["start"]
                    )
...
def recover_active_rentals(bot):
    ...
    for r in rentals:
        ...
        # Eğer hala süre varsa, SMS takip thread'ini kalan süreyle yeniden başlat!
        print(f"[+] Kurtarma: Kiralama aktif, kalan süre: {int(remaining)} sn. Takip yeniden başlatılıyor...")
        
        with tracking_lock:
            AKTIF_TAKIPLER[act_id] = {"status": True, "user_id": user_id, "start": start_time}
            
        threading.Thread(target=live_track_sms, args=(bot, chat_id, msg_id, act_id, price, svc_name, api_cc, phone, user_id, sid, api_srv, prov, start_time), daemon=True).start()