        ...
        if int(time.time()) % 4 == 0:
            try:
                ...
                if isinstance(sonuc, str) and sonuc != "WAIT_CODE":
                    ...
                    # Log kaydını servisin yalın adı (Popüler menüsünün doğru sayması için) ve birleşik veriyle at
                    database.add_to_history(user_id, 2, svc_name.capitalize(), ulke_numara_kod, fiyat, status="✅ BAŞARILI")
...
def callback_yonetici(call):
    ...
    if data == "back_to_main":
        ...
        try:
            ...
            # Veritabanında action_type = 2 (Başarılı Alım) olan ve en çok tekrarlanan ilk 5 servisi çek
            sorgu = """
                SELECT service_name, COUNT(*) as alim_sayisi 
                FROM history 
                WHERE action_type = 2 
            ...
            if not populer_listesi:
                ...
                for srv in populer_listesi:
                    ...
                    markup.add(types.InlineKeyboardButton(f"🔥 {srv_name.upper()} ({srv['alim_sayisi']} Kez Alındı)", callback_data=f"select_srv_{srv_name}"))
