...
def add_to_history(user_id, action_type, service_name, fake_number, price, status="✅ BAŞARILI"):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO history (user_id, action_type, service_name, fake_number, price, status) VALUES (%s, %s, %s, %s, %s, %s)",
        (user_id, action_type, service_name, fake_number, price, status)
    )
    conn.commit()
    conn.close()
...
def ban_user(user_id, ban_type=0, reason=None, input_data=None):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE users SET is_banned = TRUE, ban_type = %s, ban_reason = %s, banned_input = %s WHERE user_id = %s", (ban_type, reason, input_data, user_id))
    conn.commit()
    conn.close()
